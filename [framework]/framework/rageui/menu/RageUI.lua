...
function RageUI.CloseAll()
	...
	if RageUI.CurrentMenu ~= nil then
		...
		while parent ~= nil do
			...
			parent.Pagination.Maximum = RageUI.MaxPagination
			parent = parent.Parent
		end

		RageUI.CurrentMenu.Index = 1
		RageUI.CurrentMenu.Pagination.Minimum = 1
		RageUI.CurrentMenu.Pagination.Maximum = RageUI.MaxPagination
		...
		if Enabled then
			if RageUI.CurrentMenu ~= nil then
				if RageUI.CurrentMenu() then
					...
					RenderText(RageUI.CurrentMenu.Title, RageUI.CurrentMenu.X + RageUI.Settings.Items.Title.Text.X + (RageUI.CurrentMenu.WidthOffset / 2), RageUI.CurrentMenu.Y + (type(RageUI.CurrentMenu.Title) == 'string' and (#RageUI.CurrentMenu.Title > 22 and (RageUI.Settings.Items.Title.Text.Y * 2) or RageUI.Settings.Items.Title.Text.Y) or RageUI.Settings.Items.Title.Text.Y), 1, (type(RageUI.CurrentMenu.Title) == 'string' and (#RageUI.CurrentMenu.Title > 22 and (RageUI.Settings.Items.Title.Text.Scale / 2) or RageUI.Settings.Items.Title.Text.Scale) or RageUI.Settings.Items.Title.Text.Scale), 255, 255, 255, 255, 1)
...
function RageUI.ItemsDescription(CurrentMenu, Description, Selected)
	...
	if Description ~= "" or Description ~= nil then
		...
		if Selected and CurrentMenu.Description ~= Description then
			...
			-- count if it has new line breaks

			if Description then
				if string.find(Description, "NL") then
					local _, count = string.gsub(Description, "%-NL", "")

					DescriptionLineCount = math.floor(DescriptionLineCount + (count / 2))
				end

				if string.find(Description, "~n~") then
					local _, count = string.gsub(Description, "%~n~", "")
			...
			CurrentMenu.DescriptionHeight = CurrentMenu.DescriptionHeight + 1.3
		end
	end
end
...
function RageUI.ItemsMouseBounds(CurrentMenu, Selected, Option, SettingsButton)
	---@type boolean
	local Hovered = RageUI.IsMouseInBounds(CurrentMenu.X + CurrentMenu.SafeZoneSize.X, CurrentMenu.Y + SettingsButton.Rectangle.Y + CurrentMenu.SafeZoneSize.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.Rectangle.Width + CurrentMenu.WidthOffset, SettingsButton.Rectangle.Height)
...
function RageUI.RefreshPagination()
	if (RageUI.CurrentMenu ~= nil) then
		if (RageUI.CurrentMenu.Index > RageUI.MaxPagination) then
			local offset = RageUI.CurrentMenu.Index - RageUI.MaxPagination
			RageUI.CurrentMenu.Pagination.Minimum = 1 + offset
			RageUI.CurrentMenu.Pagination.Maximum = RageUI.MaxPagination + offset
		else
			RageUI.CurrentMenu.Pagination.Minimum = 1
			RageUI.CurrentMenu.Pagination.Maximum = RageUI.MaxPagination
		end
