        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            if Enabled then
                ...
                if Style.RightLabel then
                    ...
                    local temp_label_name = Style.RightLabel

                    if Active then
                        if type(temp_label_name) ~= 'number' then
                            temp_label_name = temp_label_name:gsub("~[a-zA-Z]~", "")
                        end
                    end

                    RenderText(temp_label_name, CurrentMenu.X + SettingsButton.RightText.X - RightBadgeOffset + CurrentMenu.WidthOffset, CurrentMenu.Y + SettingsButton.RightText.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, SettingsButton.RightText.Scale, Active and 0 or LabelColor.R, Active and 0 or LabelColor.G, Active and 0 or LabelColor.B, 255, 2)
                end

                local temp_label_name = Label
                ...
                RenderText(temp_label_name, CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, SettingsButton.Text.Scale, Active and 0 or 245, Active and 0 or 245, Active and 0 or 245, 255)
            else
                if haveRightBadge then
