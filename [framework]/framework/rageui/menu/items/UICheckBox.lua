...
---@return table
local function CheckBoxLockBadgeColor(Selected) -- luacheck: ignore
    if Selected then
        return 0, 0, 0, 255
    else
...
---@return nil
---@public
function RageUI.Checkbox(Label, Description, Checked, Style, Callback, onChecked, onUnchecked, muteSound)
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                if type(Style) == "table" then
                    ...
                    if Selected and (CurrentMenu.Controls.Select.Active or (Hovered and CurrentMenu.Controls.Click.Active)) and (Style.Enabled == true or Style.Enabled == nil) then
                        ...
                        if not muteSound then RageUI.PlaySound(Audio[Audio.Use].Select.audioName, Audio[Audio.Use].Select.audioRef) end
                        Checked = not Checked
                        if (Checked) then
                    ...
                    if Selected and (CurrentMenu.Controls.Select.Active or (Hovered and CurrentMenu.Controls.Click.Active)) and (Style.Enabled == false) then
                        local Audio = RageUI.Settings.Audio
                        if not muteSound then RageUI.PlaySound(Audio[Audio.Use].Error.audioName, Audio[Audio.Use].Error.audioRef) end
                        Checked = false
                        if (Checked) then
                            if (onChecked ~= nil) then
                                onChecked();
